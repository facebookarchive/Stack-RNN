...
/*
 * This class is the same as StackRNN
 * with a hierarchical softmax instead of a softmax classifier.
 * This is use in the language modelling experiments for speed (see train_word.cpp).
 */
...
namespace rnn{
  ...
  struct StackHRNN{
    public:
      ...
      StackHRNN(const std::string& filename){
        load(filename);
        emptyStacks();
      }
      ...
      StackHRNN(my_int si,
          ...
          my_int sm,
          my_int bptt_step,
          std::vector<my_int> pos,
          my_int mod = 1,
          bool isnoop = false,
          bool is_act_discrete = false,
          my_int depth = 1) :
        ...
        _DEPTH(depth),
        _in2hidTranspose(_HIDDEN, _IN), // in2hid weights
        ...
        _out(_BPTT, Vec(_OUT + _CLASS, 0)),
        _targets(_BPTT, 0),
        _pos(pos),
        _err_out(_OUT + _CLASS, 0),
        _err_hid (_HIDDEN, 0),
      ...
      my_int getclassidx(const my_int& target){
        for(my_int i = 0; i < _pos.size()-1; i++)
          if(target < _pos[i+1])
            return i;
        return -1;
      }
      ...
      void forward(const my_int& cur, const my_int& target, bool ishard = false){
        ...
        my_int cl = getclassidx(target);
        my_int b = _pos[cl], e = _pos[cl+1];

        _isemptied[_it_mem] = false;

        // zeros the current hidden states:
        for(my_int i = _OUT; i < _OUT + _CLASS; i++) _out[_it_mem][i] = 0;
        for(my_int i = b; i < e; i++) _out[_it_mem][i] = 0;
        _hid[_it_mem].zeros();
        for(my_int s = 0; s <_NB_STACK; s++){
        ...
        _hid2out.forward(_hid[_it_mem], _out[_it_mem],0, _HIDDEN, _OUT, _OUT + _CLASS);
        // elements of the given class:
        _hid2out.forward(_hid[_it_mem], _out[_it_mem], 0, _HIDDEN, b, e);

        Softmax::forward(_out[_it_mem], _OUT, _OUT + _CLASS);
        Softmax::forward(_out[_it_mem], b, e);
      }
      ...
      void backward(){
        ...
        for(my_int i = _OUT; i < _OUT + _CLASS; i++)
          for(my_int j = 0; j < _HIDDEN; j++)
            _hid2out._gradient(i, j) = 0;

        my_int cl,b,e;
        for(my_int t = 0; t < _targets.size(); t++){
          cl = getclassidx(_targets[t]);
          b = _pos[cl]; e = _pos[cl+1];
        ...
        while(count < _BPTT){
          ...
          if( count < _BPTT_STEP){
            ...
            cl = getclassidx(_targets[itm]);
            b = _pos[cl]; e = _pos[cl+1];

            //classes
            for(my_int i = _OUT; i < _OUT + _CLASS; i++){
              _err_out[i] = -_out[itm][i];
            }
            _err_out[_OUT + cl] += 1;

            // element in the target class:
            for(my_int i = b; i < e; i++)
              _err_out[i] = -_out[itm][i];
            _err_out[_targets[itm]] +=1;

            // Compute gradient from hidden -> out
            _hid2out.computeGradient(_hid[itm], _err_out,0, _HIDDEN, _OUT, _OUT +_CLASS);
            _hid2out.computeGradient(_hid[itm], _err_out, 0, _HIDDEN, b, e);

            //propagate error from out -> hidden
            _hid2out.backward(_err_hid, _err_out, 0, _HIDDEN, _OUT, _OUT+_CLASS);
            _hid2out.backward(_err_hid, _err_out, 0, _HIDDEN, b, e);
          ...
          for(my_int s = 0; s <_NB_STACK;s++){
            ...
            // propagate error on action through non-linearity:
      ...
      void update(const my_real& lr){
        ...
        for(my_int i = _OUT; i < _OUT + _CLASS; i++)
          for(my_int j = 0; j < _HIDDEN; j++)
            _hid2out._data(i,j) += lr * _hid2out._gradient(i,j);


        std::vector<bool> isclvisited(_CLASS, false);

        my_int cl;
        for(my_int t = 0; t < _BPTT; t++){
          cl = getclassidx(_targets[itm]);
          if(itm != 0) itm -= 1;
          else itm = _in.size() - 1;
          if(isclvisited[cl]) continue;
          isclvisited[cl] = true;
          my_int b = _pos[cl], e = _pos[cl+1];
          for(my_int i = b; i < e; i++)
            for(my_int j = 0; j < _HIDDEN; j++){
        ...
        if(_mod == 2){
          for(my_int i =0; i < _hid2hid.size(); i++)
            _hid2hid._data[i] += lr * _hid2hid._gradient[i];
        }

        for(my_int s = 0; s <_NB_STACK;s++){
          for(my_int i =0; i < _hid2act[s].size(); i++)
            _hid2act[s]._data[i] += lr * _hid2act[s]._gradient[i]  ;


          for(my_int i =0; i < _stack2hid[s].size(); i++)
            _stack2hid[s]._data[i] += lr * _stack2hid[s]._gradient[i];


          for(my_int i =0; i < _hid2stack[s].size(); i++)
            _hid2stack[s]._data[i] += lr * _hid2stack[s]._gradient[i];
        }

        // update in -> hidden:
        for(my_int i =0; i < _in2hidTranspose.size(); i++)
          _in2hidTranspose._data[i] += lr * _in2hidTranspose._gradient[i];
      }

      my_real eval(const my_int& target){
        return  _out[_it_mem][getclassidx(target) + _OUT] * _out[_it_mem][target];
      }
      ...
      void pretty_print(std::string filename){
        std::string filename_pretty = filename + "-pretty";
        FILE* f;
        f= fopen(filename_pretty.c_str(),"w");
        fprintf(f, "IN2HIDDEN matrix of size %dx%d\n", _HIDDEN,_IN);
        for(my_int i = 0; i < _HIDDEN; i++){
          for(my_int j = 0; j < _IN; j++)
            fprintf(f, "%3.3f  ", _in2hidTranspose._data(j,i));
          fprintf(f, "\n");
        }
        fprintf(f, "\n");
        fprintf(f, "_HIDDEN2HIDDEN matrix of size %dx%d\n", _HIDDEN,_HIDDEN);
        for(my_int i = 0; i < _HIDDEN; i++){
          for(my_int j = 0; j < _HIDDEN; j++)
            if( _hid2hid._data(i,j) != 0)
              fprintf(f, " Alert non-zero entry at %d %d: %f\n", i,j, _hid2hid._data(i,j));
        }
        fprintf(f, "\n");
        fprintf(f, "_HIDDEN2OUT matrix of size %dx%d\n", _OUT,_HIDDEN);
        for(my_int i = 0; i < _OUT; i++){
          for(my_int j = 0; j < _HIDDEN; j++)
            if( _hid2out._data(i,j) != 0)
              fprintf(f, "%3.3f  ", _hid2out._data(i,j));
          fprintf(f, "\n");
        }
        fprintf(f, "\n");
        for(my_int s = 0; s <_NB_STACK;s++){
          fprintf(f, "_HIDDEN2STACK %d matrix of size %dx%d\n", s, _STACK_SIZE, _HIDDEN);
          for(my_int i = 0; i < _STACK_SIZE; i++){
            for(my_int j = 0; j < _HIDDEN; j++)
              fprintf(f, "%3.3f  ", _hid2stack[s]._data(i,j));
            fprintf(f, "\n");
          }
          fprintf(f, "\n");
          fprintf(f, "_HIDDEN2ACTION %d matrix of size %dx%d\n", s, _ACTION, _HIDDEN);
          for(my_int i = 0; i < _ACTION; i++){
            for(my_int j = 0; j < _HIDDEN; j++)
              fprintf(f, "%3.3f  ", _hid2act[s]._data(i,j));
            fprintf(f, "\n");
          }
          fprintf(f, "\n");
          fprintf(f, "_STACK2HID %d matrix of size %dx%d\n", s, _HIDDEN, _STACK_SIZE);
          for(my_int i = 0; i < _HIDDEN; i++){
            for(my_int j = 0; j < _STACK_SIZE; j++)
              fprintf(f, "%3.3f  ", _stack2hid[s]._data(i,j));
            fprintf(f, "\n");
          }
          fprintf(f, "\n");
      ...
      void save(std::string filename){
        ...
        for(my_int i = 0; i < _in2hidTranspose.size(); i++)
          fprintf(f, "%f,", _in2hidTranspose._data[i]);
        for(my_int i = 0; i < _hid2hid.size(); i++)
          fprintf(f, "%f,", _hid2hid._data[i]);
        for(my_int s = 0; s <_NB_STACK;s++){
          for(my_int i = 0; i < _hid2act[s].size(); i++)
            fprintf(f, "%f,", _hid2act[s]._data[i]);
          for(my_int i = 0; i < _hid2stack[s].size(); i++)
            fprintf(f, "%f,", _hid2stack[s]._data[i]);
          for(my_int i = 0; i < _stack2hid[s].size(); i++)
            fprintf(f, "%f,", _stack2hid[s]._data[i]);
        }
        for(my_int i = 0; i < _hid2out.size(); i++)
          fprintf(f, "%f,", _hid2out._data[i]);
        fclose(f);
      }
      ...
      void load(const std::string& filename){
        ...
        _pred_err_stack = std::vector<Vec > (
            _NB_STACK, Vec(_STACK_SIZE,0));
        for(my_int i = 0; i < _in2hidTranspose.size(); i++)
          fscanf(f, "%lf,", &_in2hidTranspose._data[i]);
        for(my_int i = 0; i < _hid2hid.size(); i++)
          fscanf(f, "%lf,", &_hid2hid._data[i]);
        for(my_int s = 0; s <_NB_STACK;s++){
          for(my_int i = 0; i < _hid2act[s].size(); i++)
            fscanf(f, "%lf,", &_hid2act[s]._data[i]);
          for(my_int i = 0; i < _hid2stack[s].size(); i++)
            fscanf(f, "%lf,", &_hid2stack[s]._data[i]);
          for(my_int i = 0; i < _stack2hid[s].size(); i++)
            fscanf(f, "%lf,", &_stack2hid[s]._data[i]);
        }
        for(my_int i = 0; i < _hid2out.size(); i++)
          fscanf(f, "%lf,", &_hid2out._data[i]);
